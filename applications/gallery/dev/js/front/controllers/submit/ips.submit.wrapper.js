...
;( function($, _, undefined){
	...
	ips.controller.register('gallery.front.submit.wrapper', {
		...
		setUpImageDetailsForm: function (e) {
			...
			let imageId;
			let tempId;
			const detailsPanel = this.scope.find('[data-role="imageDetails"]');

			// Get the image ID first
			if ($(e.currentTarget).attr('data-uploading-temp-id')) {
				// if the details pane was opened before the fileid was set, we use the existing temp id
				imageId = $(e.currentTarget).attr('data-uploading-temp-id')
			} else if( $( e.currentTarget ).attr('data-fileid').startsWith('o_') ) {
				imageId = $('input[name="images_existing\\[' + $( e.currentTarget ).attr('data-fileid') + '\\]"').val();
				if (!imageId) {
					imageId = $(e.currentTarget).attr('data-fileid');
					$(e.currentTarget).attr('data-uploading-temp-id', $(e.currentTarget).attr('data-fileid'))
				}
			} else if ( $(e.currentTarget).attr('data-fileid') && parseInt($(e.currentTarget).attr('data-fileid')) ) {
				imageId = $( e.currentTarget ).attr('data-fileid');
			} else {
				// in this case, we assume it's still uploading
				tempId = imageId = Math.round(Math.random() * Math.pow(2, 32)).toString(16)
				$(e.currentTarget).attr('data-uploading-temp-id', imageId)
			}
				...
				if( !_.isUndefined( imagePreview ) ){
					imageForm
						.find('.cGallerySubmit_preview')
							...
							.html( $("<img class='ipsImage'/>").attr('src', imagePreview ) )
							.show();
				}
				else {
					imageForm
		...
		submitForm: function (e) {
			...
			if( form.attr('id') == 'elGallerySubmit' ){
				...
				const tempIdMap = {}
				form.find('[data-uploading-temp-id][data-fileid]').each(function() {
					tempIdMap[$(this).attr('data-uploading-temp-id')] = $(this).attr('data-fileid');
				})

				form.find('textarea[name="images_info"]').val( JSON.stringify( $('#form_imageDetails').serializeArray().map(({name, value}) => {
					for(const key in tempIdMap) {
						if (name.includes(key)) {
							name = name.replaceAll(key, tempIdMap[key])
							break;
						}
					}
					return {name, value}
