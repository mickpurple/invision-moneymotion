	    ...
	    editor.widgets.add( 'ipsquote', {
		    ...
		    upcast: function( element ) {
		        if ( element.name == 'blockquote' && element.hasClass( 'ipsQuote' ) ) {
					/* Disable grammarly - it causes issues - test re-enable 4.7.7 */
					...
					if( typeof element.children[0].hasClass === "function" )
					{
				    ...
				    if ( selectedElement.getName() == 'p' && !paragraphContent ) {
					    ...
					    if ( quoteAscendant ) {
							...
							if ( selectedElement.getNext() ) {
								...
								var originalQuote = quoteAscendant.getParent();

								if( originalQuote.$.hasAttributes() ){
									var originalAttr = originalQuote.$.attributes;
									
									// Loop through the attributes on the original quote and match any of our quote widget ones so that
									// we can apply them to the new quote too.
									for( var x = originalAttr.length - 1; x >= 0; x--){
										if( originalAttr[x].specified && originalAttr[x].name.startsWith('data-ipsquote') ){
											newQuote.setAttribute( originalAttr[x].name, originalAttr[x].value );
										}
									}
								}
