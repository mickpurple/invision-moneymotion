...
;( function($, _, undefined){
	...
	ips.controller.register('core.admin.core.changeTheme', {
		...
		initialize: function () {
			$('#elAdminUser').on( 'menuItemSelected', _.bind( this.themePreferenceSelected, this ) );
		...
		themePreferenceSelected: function (e, data) {
			...
			$('#elThemeMenu_menu').find('.ipsMenu_itemChecked').removeClass('ipsMenu_itemChecked');
			$('#elThemeMenu_menu').find( '[data-ipsMenuValue="' + data.selectedItemID + '"]' ).addClass('ipsMenu_itemChecked');
